...
class AuthController extends Controller
{
    ...
    {
        return response()->json(auth()->user()); //kthen userin e kyqt 
    }
    ...
    {
        try {
            $oldToken = JWTAuth::getToken(); //token qe ekziston
            $newToken = auth()->refresh(); //token new 

            if($oldToken) {
                try {
                    JWTAuth::invalidate($oldToken);
                } catch (\Exception $e) {
                    \Log::warning("Token could not beinvalidated");
                }
            }
